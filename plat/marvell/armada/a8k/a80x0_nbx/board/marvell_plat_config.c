 ...
 *
 * NBX board uses UART-based skip image recovery.
 * Holding the 's' key during boot will skip to secondary image.
 * GPIO pin 33 is also available as a fallback.
 *****************************************************************************
 */
#if PLAT_RECOVERY_IMAGE_ENABLE

/*
 * UART skip image check - NBX specific
 * Check if 's' character is present in UART receive buffer.
 * This allows recovery by holding 's' key during boot.
 */
static int ap_uart_skip_check(struct skip_image *im)
{
	int i;
	...
	for (i = 0; i < 512; ++i) {
		...
		/* Check if data is ready (LSR bit 0) before reading RBR */
		lsr = mmio_read_32(PLAT_MARVELL_UART_BASE + UARTLSR);
		if ((lsr & UARTLSR_RDR) != 0U) {
			uint32_t rbr;

			rbr = mmio_read_32(PLAT_MARVELL_UART_BASE + UARTRX) & 0xff;
			if (rbr == 's')
				return 1;
		}
	}
...
struct skip_image skip_im = {
	...
	.info.user_defined.check = ap_uart_skip_check,
};
