 ...
 * Crptography
 */
struct __packed sfcp_cryptography_config_t {
	uint16_t seq_num;
	enum sfcp_cryptography_mode_t mode;
	uint8_t trusted_subnet_id;
};

struct __packed sfcp_cryptography_metadata_t {
...
#define GET_METADATA_FIELD(_field, _metadata)          \
	(((_metadata) >> METADATA_##_field##_OFFSET) & \
	 (METADATA_##_field##_MASK))
#define SET_METADATA_FIELD(_field, _value) \
	(((_value) & METADATA_##_field##_MASK) << (METADATA_##_field##_OFFSET))
#define SET_ALL_METADATA_FIELDS(_packet_type, _uses_crypto, _uses_id, \
				_protocol_version)                    \
 ...
 */
struct __packed sfcp_id_extension_payload {
	union __packed {
		struct __packed {
			uint16_t client_id;
...
struct __packed sfcp_packet_t {
	struct sfcp_header_t header;
	union __packed {
		struct __packed {
			struct sfcp_cryptography_metadata_t
				cryptography_metadata;
...
#define GET_SFCP_ID_EXTENSION_PAYLOAD(_packet, _uses_crypto)          \
	((_uses_crypto) ?                                             \
		 &(_packet)->cryptography_used.id_extension_payload : \
		 &(_packet)->cryptography_not_used.id_extension_payload)
#define GET_SFCP_GENERIC_FIELD_PTR(_packet, _uses_crypto, _uses_id_extension, \
				   _field_name)                               \
	((_uses_id_extension) ?                                               \
		 &GET_SFCP_ID_EXTENSION_PAYLOAD(_packet, _uses_crypto)        \
...
#define GET_SFCP_PAYLOAD_PTR(_packet, _uses_crypto, _uses_id_extension)       \
	GET_SFCP_GENERIC_FIELD_PTR(_packet, _uses_crypto, _uses_id_extension, \
				   payload)
...
#define GET_SFCP_ERROR_REPLY_FIELD(_packet, _field) \
	((_packet)->error_reply._field)
#define GET_SFCP_ERROR_REPLY_CLIENT_ID(_packet) \
	GET_SFCP_ERROR_REPLY_FIELD(_packet, client_id)
	...
	((_uses_crypto) ?                                                     \
		 ((_uses_id_extension) ?                                      \
			  offsetof(struct sfcp_packet_t,                      \
				   cryptography_used.id_extension_payload     \
					   .id_extension_used.payload) :      \
			  offsetof(struct sfcp_packet_t,                      \
				   cryptography_used.id_extension_payload     \
					   .id_extension_not_used.payload)) : \
		 ((_uses_id_extension) ?                                      \
			  offsetof(struct sfcp_packet_t,                      \
				   cryptography_not_used.id_extension_payload \
					   .id_extension_used.payload) :      \
			  offsetof(struct sfcp_packet_t,                      \
				   cryptography_not_used.id_extension_payload \
					   .id_extension_not_used.payload)))
#define SFCP_PACKET_SIZE_ERROR_REPLY                   \
	(offsetof(struct sfcp_packet_t, error_reply) + \
	 sizeof(((struct sfcp_packet_t *)0)->error_reply))
