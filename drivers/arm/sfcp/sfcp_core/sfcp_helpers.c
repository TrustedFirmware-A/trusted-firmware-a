			  ...
			  enum sfcp_packet_type_t *packet_type)
{
	...
	*uses_id_extension =
	...
	case SFCP_PACKET_TYPE_REPLY:
		if (packet_size <
		    SFCP_PACKET_SIZE_WITHOUT_PAYLOAD(*uses_cryptography,
						     *uses_id_extension)) {
			return SFCP_ERROR_INVALID_PACKET_SIZE;
		}
		...
		*needs_reply =
		...
		if (*uses_id_extension) {
			*client_id =
				GET_SFCP_CLIENT_ID(packet, *uses_cryptography);
			*application_id = GET_SFCP_APPLICATION_ID(
				packet, *uses_cryptography);
		} else {
			*client_id = 0;
			*application_id = 0;
		}
		...
		*payload_len = packet_size -
		...
		if (*payload_len > 0) {
			*payload = (uint8_t *)GET_SFCP_PAYLOAD_PTR(
				packet, *uses_cryptography, *uses_id_extension);
		} else {
			*payload = NULL;
		}
...
bool sfcp_helpers_packet_requires_forwarding_get_destination(
	sfcp_node_id_t sender, sfcp_node_id_t receiver,
	enum sfcp_packet_type_t packet_type, sfcp_node_id_t my_node_id,
	sfcp_node_id_t *destination)
{
	bool msg_requires_forwarding, reply_requires_forwarding;

	msg_requires_forwarding =
...
void sfcp_helpers_generate_protocol_error_packet(
	struct sfcp_packet_t *packet, sfcp_node_id_t sender_id,
	sfcp_node_id_t receiver_id, sfcp_link_id_t link_id, uint16_t client_id,
	uint8_t message_id, enum sfcp_protocol_error_t error)
{
	packet->header.metadata =
		SET_ALL_METADATA_FIELDS(SFCP_PACKET_TYPE_PROTOCOL_ERROR_REPLY,
					false, false, SFCP_PROTOCOL_VERSION);
...
enum sfcp_error_t sfcp_helpers_drop_receive_message(sfcp_link_id_t link_id,
						    size_t message_size,
						    size_t already_received)
{
	...
	while (remaining > 0) {
		...
		hal_err = sfcp_hal_receive_message(link_id, drop_message_buf,
						   message_size,
