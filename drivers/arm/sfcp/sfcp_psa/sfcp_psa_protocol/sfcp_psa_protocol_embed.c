...
#include "sfcp_psa_protocol_common.h"
#include "sfcp_psa_protocol_embed.h"

psa_status_t
sfcp_protocol_embed_serialize_msg(psa_handle_t handle, int16_t type,
				  ...
				  struct sfcp_embed_msg_t *msg, size_t *msg_len)
{
	...
	for (i = 0U; i < in_len; ++i) {
		...
		memcpy(msg->payload + payload_size, in_vec[i].base,
		       in_vec[i].len);
	...
	const struct sfcp_embed_reply_t *reply, size_t reply_size)
{
	...
	for (i = 0U; i < out_len; ++i) {
		if ((sizeof(*reply) - sizeof(reply->payload) + payload_offset) >
		    reply_size) {
			return PSA_ERROR_INVALID_ARGUMENT;
		}

		memcpy(out_vec[i].base, reply->payload + payload_offset,
		       reply->out_size[i]);
