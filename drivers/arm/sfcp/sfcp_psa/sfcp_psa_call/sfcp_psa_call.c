...
static enum sfcp_protocol_version_t
select_protocol_version(const psa_invec *in_vec, size_t in_len,
		      ...
		      psa_outvec *out_vec, size_t out_len)
{
	...
	struct serialized_psa_reply_t *psa_reply = NULL;
	size_t psa_msg_len;
	size_t payload_len;
	...
	sfcp_err = sfcp_init_msg(psa_payload_buf, sizeof(psa_payload_buf),
				 SFCP_PSA_SERVER_NODE_ID,
				 SFCP_PSA_SERVER_APPLICATION_ID, client_id,
				 true, false, 0, (uint8_t **)&psa_msg,
				 &payload_len, &msg, &msg_size, &metadata);
	...
	status = sfcp_protocol_serialize_msg(handle, type, in_vec, in_len,
					     out_vec, out_len, psa_msg,
					     &psa_msg_len);
	...
	do {
		sfcp_err = sfcp_receive_reply(psa_payload_buf,
					      sizeof(psa_payload_buf), metadata,
					      (uint8_t **)&psa_reply,
					      &payload_len);
	...
	VERBOSE("[SFCP] Received reply\n");
	VERBOSE("protocol_ver=%u\n", psa_reply->header.protocol_ver);

	status = sfcp_protocol_deserialize_reply(out_vec, out_len, &return_val,
						 psa_reply, payload_len);
	if (status != PSA_SUCCESS) {
		return status;
	}
