...
stage. The boot process takes advantage of both SRAM and DDR memory. BootROM
loads BL2 into SRAM, and BL2 is responsible for initializing DDR and loading
subsequent stages (such as BL31 and BL33) into their designated memory regions.
To create a bootable image, the script below should be used.
This script makes use of the ``mkimage`` tool, which is part of the U-Boot drop
for S32G274A SoCs.

.. warning::
   The ``DDR_FW_BIN_PATH`` parameter must point to the location of the folder
   containing the DDR firmware binary specific to the S32G274ARDB2 platform.
   This binary is essential for proper DDR initialization. It is included in the
   Linux BSP releases for the S32G274A-RDB board and can be obtained from https://www.nxp.com/.
